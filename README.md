...
- **Evaluation + observability across five axes** — a 67-email golden dataset runs as LangSmith experiments per model/prompt version: exact-set accuracy (91.0%), task splitting (100%), forward conventions (100%), plus calibrated LLM judges for faithfulness (97.0%) and instruction-following (97.4%), with latency/token/cost traces. Includes a worked lesson in judge calibration ([docs/evals.md](docs/evals.md)).
