...
import type { GmailClient } from "../src/lib/gmail";
...
const FIXTURES: Fixture[] = [
  {
    ...
    snap: { threadId: "demo-1", from: "DB Agent Copy <ny_agent_copy@dxc.com>", subject: "Agent Copy of Print - Policy DP2201984",
      listId: null, attachments: ["DP2201984.pdf"], bodyText: "Attached is the agent copy of the policy print.", internalDateMs: 1 },
  ...
  },
  {
    ...
    snap: { threadId: "demo-2", from: "Policy Services <policyservices@lighthouse-mutual.example>", subject: "Cancellation Endorsement - Policy CP8811223 - RIVERSIDE HARDWARE LLC",
      listId: null, attachments: ["cancellation-endorsement.pdf"], bodyText: "Please find the cancellation endorsement effective 09/15 for nonpayment of premium.", internalDateMs: 2 },
  ...
  },
  {
    ...
    snap: { threadId: "demo-3", from: "Amy Torres <amy@harborpoint-ins.example>", subject: "Loss runs needed - GOLDEN WOK RESTAURANT INC",
      listId: null, attachments: [], bodyText: "Hi team, could you send 3 years of loss runs for the above insured? Renewal marketing.", internalDateMs: 3 },
  ...
  },
  {
    ...
    snap: { threadId: "demo-4", from: "CSR Desk <csr@midtowncoverage.example>", subject: "C105 for GREEN GARDEN DELI CORP WWC1122334",
      listId: null, attachments: ["cert-holder.png"], bodyText: "Please provide C-105.2 with the below certificate holder. Thank you.", internalDateMs: 4 },
  ...
  },
  {
    ...
    snap: { threadId: "demo-5", from: "AR <billing@lighthouse-mutual.example>", subject: "Commission statement and invoice - August",
      listId: null, attachments: ["invoice-0826.pdf"], bodyText: "Your monthly statement is attached. Amount due: $4,120.55 by 09/01.", internalDateMs: 5 },
  ...
  },
  {
    ...
    snap: { threadId: "demo-6", from: "Gina Park <gina@queensbridge-brokers.example>", subject: "MAPLE CLEANERS - address change + cancel BOP",
      listId: null, attachments: ["signed-LPR.pdf"], bodyText: "Two things: update the mailing address to 44-02 Main St, and cancel the BOP effective 9/30 - signed LPR attached.", internalDateMs: 6 },
    canned: { tasks: [
  ...
  },
  {
    ...
    snap: { threadId: "demo-7", from: "Leo Chan <leo@brightpath-brokerage.example>", subject: "NHO - add mortgagee clause - 88 GARDEN AVE",
      listId: null, attachments: [], bodyText: "Please add the mortgagee clause below to the homeowners policy and send the updated dec page.", internalDateMs: 7 },
  ...
  },
  {
    ...
    snap: { threadId: "demo-8", from: "Insurance Weekly <news@insuranceweekly.example>", subject: "5 trends reshaping commercial lines",
      listId: "<news.insuranceweekly.example>", attachments: [], bodyText: "This week in insurance: markets, MGAs, and more. Unsubscribe anytime.", internalDateMs: 8 },
  ...
  },
  {
    ...
    snap: { threadId: "demo-9", from: "info@oldclient.example", subject: "question about my policy",
      listId: null, attachments: [], bodyText: "Hi, I had a question about what my policy covers, can someone call me back?", internalDateMs: 9 },
    canned: { tasks: [{ labels: ["2-NY"], forward_to: "none" }], confidence: "medium", rationale: "Unclear request; front-office judgment needed." },
  },
];

function fakeGmail(log: string[]): GmailClient {
  return {
    listNewThreads: async () => [],
    applyLabels: async (id, labels) => { log.push(`  gmail.applyLabels   ${id}  [${labels.join(", ")}]`); },
    forward: async (id, to) => { log.push(`  gmail.forward       ${id}  -> ${to}`); },
    sendAlert: async () => { log.push("  gmail.sendAlert"); },
  };
}
